...
public class BasicMAryTree280<I> implements Container280
{
	...
	/**	
	 * Create an empty tree with specified maximum arity for nodes
	 * @timing O(1)
	...
	{
		if (isEmpty())
			throw new ContainerEmpty280Exception("Cannot obtain root of an empty tree");
	...
	{
		if (isEmpty())
			throw new ContainerEmpty280Exception("Cannot call count() on an empty tree");
	...
	{
		if (isEmpty())
			throw new ContainerEmpty280Exception("Cannot perform a search for the i'th subtree on an empty tree");
		if ((i<=0) || (i>rootArity()))
	...
	{
		if (isEmpty())
			throw new ContainerEmpty280Exception("Cannot set root of an empty tree.");
	 ...
	 * @timing O(1) 
	 * @param newRoot The node to be the new root of this tree.
	 */
	...
	/**	
	 * Insert x as new root item with old tree as the 
	 * i'th subtree of the new root. 
	 * @timing O(1)
	 * @precond !((i<=0) || (i>rootArity())) 
	 * @param x item to become the new root item
	 * @param i old tree is to become the i-th subtree of the new root 
	 */
	 ...
	 * @timing O(1) when tree has size 1, otherwise  
	...
	{
		if (isEmpty())
			throw new ContainerEmpty280Exception("Cannot delete from an empty tree.");
	 ...
	 * @timing O(rootArity()) worst case (inserting empty tree)
	 * @precond !isEmpty()
	 * @precond !((i<=0) || (i>rootArity)) 
	 ...
	 * @timing O(n), n = number of items in the tree 
	 */
	 ...
	 * @timing O(n), where n = number of items in the tree 
	 */
	...
	{
		if (isEmpty())
			throw new ContainerEmpty280Exception("Cannot find rightmost leaf of an empty tree.");
